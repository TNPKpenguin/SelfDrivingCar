...
while True:
    ...
    gray_transform_img = cv2.cvtColor(transform_frame, cv2.COLOR_BGR2GRAY)
    ...
    ret, thresh = cv2.threshold(rgb, 220, 255, cv2.THRESH_BINARY)
    kernels = np.ones((7, 7), np.uint8)
    thresh2 = cv2.erode(thresh, kernels, 2)
    kernels = np.ones((300, 10), np.uint8)
